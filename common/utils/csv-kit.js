(function() {
  ...
  CSVKit.Reader = function(options) {
    ...
    this.columns_from_header =
      'columns_from_header' in options ? options.columns_from_header : true;
    this.nested_quotes =
      'nested_quotes' in options ? options.nested_quotes : false;
    this.rows = [];
          ...
          if (!is_escape && c === this.quote_char) {
            ...
            if (this.state.in_quoted_field) {
              ...
              if (
                ...
                next_char !== '\r' &&
                next_char != '\n' &&
                next_char !== this.separator &&
                this.nested_quotes !== true
              ) {
                throw new Error(
                  'separator expected after a closing quote; found ' + next_char
                );
              } else {
  ...
  CSVKit.Writer = function(options) {
    ...
    CSVKit.Writer.prototype._serialize_cell = function(cell) {
      ...
      if (
        this.quote_all ||
        cell.indexOf(this.separator) >= 0 ||
        cell.indexOf(this.newline) >= 0
      ) {
