...
class OpeningHours extends Component<Props, State> {
  state = {
    activePlace:
      this.props &&
      this.props.groupedVenues &&
      `${this.props.idPrefix || ''}${Object.keys(this.props.groupedVenues)[0]}`,
  };
  ...
  render() {
    ...
    return (
      <Fragment>
        {groupedVenues &&
          !(
            groupedVenues[Object.keys(groupedVenues)[0]].hours &&
            groupedVenues[Object.keys(groupedVenues)[0]].hours.length > 1
          ) && (
            <p className={spacing({ s: 2 }, { margin: ['bottom'] })}>
              <a
                className={font({ s: 'HNL6', m: 'HNL5' })}
                href="https://wellcomecollection.org/opening-times"
              >
                Opening times
              </a>
            </p>
        ...
        {upcomingExceptionalOpeningPeriods &&
          upcomingExceptionalOpeningPeriods.length > 0 && (
            <p className={`${font({ s: 'HNM4' })}`}>
                  ...
                  if (group.dates.length > 1) {
                    return (
                      <span key={firstDate.toString()}>
                        ...
                        {`${typeWording} between`}{' '}
                        <span className="nowrap">{formatDate(firstDate)}</span>
                        &mdash;
              ...
              . Please check our{' '}
              <a href="/opening-times#exceptional">revised opening hours</a> for
        ...
        {groupedVenues &&
          ...
          groupedVenues[Object.keys(groupedVenues)[0]].hours.length > 1 && (
            <div
              className={`opening-hours ${extraClasses ||
                ''} js-opening-hours js-tabs`}
            >
              <ul
                className={`plain-list opening-hours__tablist ${font({
                  s: 'HNM5',
                })} ${spacing(
                  { s: 0 },
                  {
              ...
              >
                {groupedVenues &&
                  Object.keys(groupedVenues).map(key => (
                    <li
                      key={key}
                      className={`opening-hours__tabitem js-tabitem ${
                        `${idPrefix || ''}${key}` === this.state.activePlace
                          ? 'opening-hours__tabitem--is-current'
                          : ''
                      }`}
                    >
                      <a
                        data-panel-id={`${idPrefix || ''}${key}`}
                        className="opening-hours__tablink js-tablink"
                        href={`#${idPrefix || ''}${key}`}
                        aria-selected={key === this.state.activePlace}
                        role="tab"
                        onClick={this.updateActivePlace}
                      >
                        {groupedVenues[key].title}
                      </a>
                    </li>
                  ))}
              </ul>
                ...
                Object.keys(groupedVenues).map(key => {
                  ...
                  return (
                    <div
                      key={id}
                      id={id}
                      className={`js-tabpanel opening-hours__panel ${
                        id === this.state.activePlace
                    ...
                    >
                      <div className="js-tabfocus">
                        <div className="is-hidden-m is-hidden-l is-hidden-xl">
                          {groupedVenues[key].hours &&
                            groupedVenues[key].hours.map(place => (
                              <OpeningHoursTable key={place.id} place={place} />
                            ))}
                        </div>
                        <div className="is-hidden-s">
                          <OpeningHoursTableGrouped
                            venues={groupedVenues[key].hours}
                          />
                        </div>
                      </div>
                    </div>
