...
const LaunchViewerButton = ({classes, clickHandler, didMountHandler}: LaunchViewerButtonProps) => {
  useEffect(() => {
    didMountHandler();
  }, []);
...
const ViewerContent = ({id, infoUrl, classes, viewerVisible, handleViewerDisplay}: ViewerContentProps) => {
    ...
    <div className={`${classes} image-viewer__content image-viewer__content2`}>
      <div className='image-viewer__controls flex flex-end flex--v-center'>
        ...
        <Control
          ...
          clickHandler={() => { handleViewerDisplay('Control'); } } />
      </div>
