...
const WorkPromo = ({ id, image, datePublished, title, link }: Props) => {
  return (
    <NextLink {...link}>
      <a
        id={id}
      ...
      >
        ...
        <div className={`promo__description ${font({ s: 'HNL5' })}`}>
          <div className="promo__heading">
            <h2
              className={`promo__title ${font({ s: 'HNL5' })} text--truncate`}
            >
              {title || `Not found`}
            </h2>
          </div>
          ...
          {datePublished && (
            <p className={`promo ${font({ s: 'HNL5' })} font-pewter relative`}>
              {datePublished}
            </p>
          )}
        </div>
