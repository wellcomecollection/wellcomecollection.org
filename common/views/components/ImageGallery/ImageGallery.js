...
const ImageGallery = ({ id, title, items }: ImageGalleryProps) => {
  return (
    <Fragment>
      ...
      <div
        className="wobbly-edge wobbly-edge--white wobbly-edge--small js-wobbly-edge"
        data-is-static="true"
      />
      <div
        className="image-gallery touch-scroll js-image-gallery"
        data-id={id}
        id={id}
      >
          ...
          >
            <CaptionedImage
              ...
              sizesQueries={
                ...
                (captionedImage.image.width / captionedImage.image.height) *
                  640 +
                'px'
              }
              preCaptionNode={
                <span
                  className={`inline-block border-right-width-1 border-color-pumice ${font(
                    { s: 'HNM5' }
                  )} ${spacing(
                    { s: 1 },
