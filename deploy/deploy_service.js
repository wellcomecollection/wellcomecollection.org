...
const services = {
  catalogue: {
    containerUrl:
      'https://registry.hub.docker.com/v2/repositories/wellcome/catalogue_webapp',
  },
  content: {
    containerUrl:
...
async function deploy(serviceName, options) {
  ...
  if (shouldDeploy) {
    ...
    const dir = exec(
      `./terraform_apply_service.sh ${serviceName} ${latestTag}`,
      (err, stdout, stderr) => {
        if (err) {
          console.error(
            colors.red(`> Terraform failed while deploying ${serviceName}`)
          );
          console.info(stderr);
        }
      }
