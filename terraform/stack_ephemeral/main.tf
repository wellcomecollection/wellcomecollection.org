...
module "catalogue" {
  ...
  container_image = "${local.catalogue_app_image}:env.next13"
  ...
  subdomain = "works.next13"
}

module "content" {
  source = "../../content/terraform/stack"

  container_image = "${local.content_app_image}:env.next13"
  nginx_image     = local.nginx_image
  env_suffix      = "next13"
  ...
  subdomain = "content.next13"
}

module "identity" {
  source = "../../identity/terraform/stack"

  container_image = "${local.identity_app_image}:env.next13"
  nginx_image     = local.nginx_image
  env_suffix      = "next13"
