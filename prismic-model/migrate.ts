...
async function init() {
  ...
  const allArticles = await client.getAllByType('articles');
  // const doc = await client.getByID('someId');
  ...
  for (const doc of allArticles) {
    await timer(1500); // don't make too many requests
