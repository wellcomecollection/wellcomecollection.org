...
const stories = async (req, res) => {
  ...
  const stories = await api.query([
    ...
    orderings: '[my.articles.publishDate, my.webcomics.publishDate, document.first_publication_date desc]',
    pageSize: 100
  });

  const rssFeed = new Rss({
    title: 'Wellcome Collection stories',
    description: 'Our words and pictures explore the connections between science, medicine, life and art. Dive into a story no matter where in the world you are.',
    feed_url: 'https://rss.wellcomecollection.org/stories',
    site_url: 'https://wellcomecollection.org/stories',
    image_url: 'https://i.wellcomecollection.org/assets/icons/android-chrome-512x512.png',
    language: 'en',
  ...
  stories.results.forEach(story => {
    const {data} = story;
    const description = data.promo && data.promo.length > 0 ? data.promo
