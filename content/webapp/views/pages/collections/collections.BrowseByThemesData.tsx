...
import { BrowseByThemeProps, ThemeCategory } from '@weco/content/webapp/types/themeBlock';
...
const CategoryLink = styled.button<{ selected: boolean }>`
  ...
  border-bottom: 2px solid ${({ selected }) => (selected ? '#007d7e' : 'transparent')};
...
const ThemeConceptCard: React.FC<{ concept: any }> = ({ concept }) => {
  const linkProps = toConceptLink({ conceptId: concept.id });
  const imageUrls = useConceptImageUrls(concept);
  return (
    <ConceptCard>
      <a {...linkProps} tabIndex={-1} aria-hidden="true" style={{ display: 'block', width: '100%' }}>
        <ImageGrid>
          {imageUrls.map((url, i) => (
            <img
              key={url + i}
      ...
      <ConceptTitle>{concept.label || concept.displayLabel || concept.id}</ConceptTitle>
      {concept.description?.text && <ConceptDescription>{concept.description.text}</ConceptDescription>}
    </ConceptCard>
  );
};
...
const getConceptsByIds = async (ids: string[]) => {
  ...
  const { getConcepts } = await import('@weco/content/services/wellcome/catalogue/concepts');
...
const BrowseByThemesData: React.FC<BrowseByThemeProps> = ({ themeConfig, initialConcepts }) => {
  const [selectedCategory, setSelectedCategory] = useState<ThemeCategory>(themeConfig.categories[0]);
  const { concepts, loading, fetchConcepts } = useThemeConcepts(initialConcepts, getConceptsByIds);
...
// All code below this line is removed to fix redeclaration and multiple export errors.
