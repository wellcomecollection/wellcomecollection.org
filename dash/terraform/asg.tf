module "dash_cluster_asg" {
  source                = "git::https://github.com/wellcometrust/terraform.git//ecs_asg?ref=v1.0.4"
  asg_name              = "dash-cluster"
  subnet_list           = "${local.vpc_subnets}"
  key_name              = "${var.key_name}"
  instance_profile_name = "${module.ecs_dash_cluster_iam.instance_profile_name}"
  user_data             = "${module.dash_cluster_userdata.rendered}"
  vpc_id                = "${local.vpc_id}"
